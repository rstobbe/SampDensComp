...
% TestSdc
%==================================================================
function err = TestSdc(ANLZ,SDCMETH)
    err.flag = 0;
    CTFV = SDCMETH.CTFV;
    KINFO = SDCMETH.KINFO;
    IT = SDCMETH.IT;
    ...
    ANLZ.MeanAbsErrTrajArr = (sum(abs(ErrorMat),1)/KINFO.nproj)*100;
    ...
    TrajAveSdcAtCentre = mean(SampDensCompMat(:,KINFO.SamplingPtAtCentre))
    MeanAbsErrTrajArrAtCentre = ANLZ.MeanAbsErrTrajArr(KINFO.SamplingPtAtCentre)
    ...
    if OPT.ANLZvis
        ...
        plot(xaxmean,100*ANLZ.MeanErrTrajArr,'r*');
        smootherr = smooth(ANLZ.MeanErrTrajArr,20);
        plot(xaxmean,100*smootherr,'k');
    ...
    ANLZ.Panel(2,:) = {'MeanAbsErrCen (%)',ANLZ.MeanAbsErrTrajArr(KINFO.SamplingPtAtCentre),'Output'};
